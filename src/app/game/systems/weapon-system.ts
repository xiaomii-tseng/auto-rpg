...
export class WeaponSystem {
  ...
  private reloadTimers: [Phaser.Time.TimerEvent | undefined, Phaser.Time.TimerEvent | undefined] = [undefined, undefined];
  ...
  onAmmoChanged?: (ammo: number, max: number) => void;
  ...
  constructor(
    ...
    this.ammo  = [this.slots[0].maxAmmo, this.slots[1].maxAmmo];
  ...
  get currentAmmo(): number { return this.ammo[this._activeSlot]; }
  ...
  switch(): void {
    ...
    this.onAmmoChanged?.(this.ammo[this._activeSlot], this.activeWeapon.maxAmmo);
  }

  /** Fire towards world point (tx, ty). Returns true if a bullet was fired. */
  fire(tx: number, ty: number): boolean {
    if (this.ammo[this._activeSlot] <= 0) return false;
    const now = this.scene.time.now;
    if (now - this.lastShotTime < this.activeWeapon.fireRate) return false;

    this.lastShotTime = now;
    this.consumeAmmo(this._activeSlot);
    this.executeAt(tx, ty);
    return true;
  }

  /** Lock on to the nearest enemy within range and fire one shot. Returns true if fired. */
  fireAtNearest(): boolean {
    const target = this.getNearestEnemy();
    if (!target) return false;
    const bc = (target.body as Phaser.Physics.Arcade.Body).center;
    return this.fire(bc.x, bc.y);
  }
  ...
  private consumeAmmo(slot: 0 | 1): void {
    this.ammo[slot]--;
    if (slot === this._activeSlot) {
      this.onAmmoChanged?.(this.ammo[slot], this.slots[slot].maxAmmo);
    }
    this.scheduleReload(slot);
  }

  private scheduleReload(slot: 0 | 1): void {
    if (this.ammo[slot] >= this.slots[slot].maxAmmo) return;
    if (this.reloadTimers[slot]) return;
    this.reloadTimers[slot] = this.scene.time.addEvent({
      delay: this.slots[slot].reloadTime,
      callback: () => {
        this.ammo[slot]++;
        this.reloadTimers[slot] = undefined;
        if (slot === this._activeSlot) {
          this.onAmmoChanged?.(this.ammo[slot], this.slots[slot].maxAmmo);
        }
        this.scheduleReload(slot);
  ...
  private executeAt(tx: number, ty: number): void {
    switch (this.activeWeapon.bulletType) {
      case BulletType.LINEAR:   this.fireLinearAt(tx, ty);   break;
      case BulletType.PIERCING: this.firePiercingAt(tx, ty); break;
      case BulletType.STRIKE:   this.fireStrikeAt(tx, ty);   break;
    }
  }

  private fireLinearAt(tx: number, ty: number): void {
    ...
    b.fire(this.player.x, this.player.y, tx, ty);
  }

  private firePiercingAt(tx: number, ty: number): void {
    const w = this.activeWeapon;

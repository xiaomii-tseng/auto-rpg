...
export class WeaponHUD {
  ...
  constructor(private readonly scene: Phaser.Scene) {
    ...
    this.slotCharTexts = [
      scene.add.text(positions[0].x + SLOT_W / 2, positions[0].y + SLOT_H / 2 - P(5), '', {
        fontSize: F(16), color: '#ffffff', stroke: '#000', strokeThickness: 3,
      }).setOrigin(0.5).setScrollFactor(0).setDepth(201),

      scene.add.text(positions[1].x + SLOT_W / 2, positions[1].y + SLOT_H / 2 - P(5), '', {
        fontSize: F(16), color: '#ffffff', stroke: '#000', strokeThickness: 3,
      }).setOrigin(0.5).setScrollFactor(0).setDepth(201),
    ];
    ...
    (['A', 'B'] as const).forEach((key, i) => {
      scene.add.text(positions[i].x + SLOT_W / 2, positions[i].y + SLOT_H - P(10), key, {
        fontSize: F(15), color: '#888888',
      }).setOrigin(0.5).setScrollFactor(0).setDepth(201);
    });

    this.namePopup = scene.add.text(d.width / 2, d.height - P(88), '', {
      fontSize: F(18), color: '#ffffff', stroke: '#000000', strokeThickness: 4,
  ...
  private positions(): [{ x: number; y: number }, { x: number; y: number }] {
    ...
    const startX = sw - (SLOT_W * 2 + SLOT_GAP) - P(16);
    return [
      { x: startX,              y: sh - SLOT_H - P(16) },
      { x: startX + SLOT_W + SLOT_GAP, y: sh - SLOT_H - P(16) },
    ];
  }
  ...
  refresh(slots: [WeaponDef, WeaponDef], activeSlot: 0 | 1): void {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      this.slotCharTexts[i]
        .setText(w.name[0])
        .setPosition(p.x + SLOT_W / 2, p.y + SLOT_H / 2 - P(5));
    }
  }

...
export class PrepScene extends Phaser.Scene {
  ...
  private showChangelog(): void {
    ...
    const ENTRIES: { text: string; header?: boolean }[] = [
      { text: 'v1.1.0', header: true },
      { text: '【新功能】新攻擊模式：雷電釋放（取代半月斬）' },
      { text: '  鎖定最近敵人持續放電，連續照射疊加增傷（×1.5→×2.0）' },
      { text: '  節點：連鎖雷電 / 快速充能 / 持久釋放 / 核爆脈衝' },
      { text: '【新功能】裝備詞綴說明面板 / 卡片說明面板' },
      { text: '【調整】五連斬重製：65px基礎範圍，每擊獨立判向最近敵人' },
      { text: '  節點：五連擴張(+50%) / 五連強化(+25%) / 雷電斬(25%落雷) / 五連迅捷(無僵直)' },
      { text: '【調整】迴旋飛刃重製：節點1範圍+30%，節點2傷害+20%，節點3連鎖彈跳' },
      { text: '【調整】地獄火節點全面重製（疊層加倍→爆炸傷害→擴散→集中）' },
      { text: '【調整】呼喚雷霆：改為攻擊命中15%機率觸發，移除200ms冷卻' },
      { text: '  5-2雷霆頻率+5% / 5-2-1雷霆共鳴+10%' },
      { text: '【調整】落雷全部限制在玩家200px內隨機敵人' },
      { text: '【調整】燃燒層數不再自然衰退，所有來源共用規則' },
      { text: '【調整】血環：傷害→最大HP×16%；節點4灼燒血環每次攻擊施加1層燃燒' },
      { text: '【調整】散射飛刀重製：攻擊10%觸發，受傷100%觸發反擊；節點6-1+5%' },
      { text: '【調整】飛刀模式：每刀獨立傷害(ATK×35%)，命中後飛行900ms可連續追打' },
      { text: '【調整】隕石術：巨型隕石移除蓄力；流星雨+巨型隕石可同時生效；傷害×1.35' },
      { text: '【調整】召喚不死花：HP×800%，15秒；節點5不死花永生延長至99秒' },
      { text: '【調整】扇形風刃：每個風刃獨立判傷' },
      { text: '【調整】藥水：回復最大HP的15%/25%/40%' },
      { text: '【調整】菁英不死花卡→充能速度+30%；不死花王卡→上限+1/充能+1/速度+60%' },
      { text: '' },
